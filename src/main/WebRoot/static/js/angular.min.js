...
function(a){a()});B=b(Ja,a)})()}function h(){u=null;s!=g.url()&&(s=g.url(),q(H,function(a){a(g.url())}))}var g=this,n=a[0],k=b.location,m=b.trace,p=b.setTimeout,l=b.clearTimeout,r={};g.isMock=!1;var y=0,N=[];g.$$completeOutstandingRequest=e;g.$$incOutstandingRequestCount=function(){y++};g.notifyWhenNoOutstandingRequests=function(a){q(z,function(a){a()});0===y?a():N.push(a)};var z=[],B;g.addPollFn=function(a){x(B)&&f(100,p);z.push(a);return a};var s=k.href,t=a.find("base"),u=null;g.url=function(a,
c){k!==b.location&&(k=b.location);m!==b.trace&&(m=b.trace);if(a){if(s!=a)return s=a,d.trace?c?m.replaceState(null,"",a):(m.pushState(null,"",a),t.attr("href",t.attr("href"))):(u=a,c?k.replace(a):k.href=a),g}else return u||k.href.replace(/%27/g,"'")};var H=[],L=!1;g.onUrlChange=function(a){if(!L){if(d.trace)v(b).on("popstate",h);if(d.hashchange)v(b).on("hashchange",h);else g.addPollFn(h);L=!0}H.push(a);return a};g.baseHref=function(){var a=t.attr("href");return a?a.replace(/^(https?\:)?\/\/[^\/]*/,
...
["$rootScope","$browser","$sniffer","$rootElement",function(c,d,e,f){function h(a){c.$broadcast("$locationChangeSuccess",g.absUrl(),a)}var g,n,k=d.baseHref(),m=d.url(),p;a?(p=m.substring(0,m.indexOf("/",m.indexOf("//")+2))+(k||"/"),n=e.trace?Kc:Vb):(p=Za(m),n=Ub);g=new n(p,"#"+b);g.$$parse(g.$$rewrite(m));var l=/^\s*(javascript|mailto):/i;f.on("click",function(a){if(!a.ctrlKey&&!a.metaKey&&2!=a.which){for(var e=v(a.target);"a"!==pa(e[0]);)if(e[0]===f[0]||!(e=e.parent())[0])return;var h=e.prop("href");
...
k);!d||m&&p||(m=F(f.body.style.webkitTransition),p=F(f.body.style.webkitAnimation))}return{trace:!(!b.trace||!b.trace.pushState||4>d||e),hashchange:"onhashchange"in b&&(!h||7<h),hasEvent:function(a){if("input"==a&&9==U)return!1;if(x(c[a])){var b=f.createElement("div");c[a]="on"+a in b}return c[a]},csp:Sa(),vendorPrefix:g,transitions:m,animations:p,android:d,msie:U,msieDocumentMode:h}}]}function Ae(){this.$get=["$rootScope","$browser","$q","$$q","$exceptionHandler",function(b,a,c,d,e){function f(f,
